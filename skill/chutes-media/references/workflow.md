...
- Override per call with `output_dir` and `filename`. Paths must remain inside the workspace, and
  filenames default to `<model>-<timestamp>.<ext>`.
- Existing files are protected by default. Set `overwrite: true` only when replacing the named asset
  is intentional.
